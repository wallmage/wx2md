...
本机已安装的命令行工具，只做这一件事：链接进，`.md` 文件出。不支持其它网站。
...
如果上面这个文件不存在，说明还没安装：运行本仓库的 `scripts/install.sh`（Windows 用 `scripts/install.ps1`），装完再调用。

参数：

| 参数 | 说明 |
| --- | --- |
| `-o, --out <路径>` | 输出目录；以 `.md` 结尾时按文件处理（只能配单个链接） |
| `-w, --image-width <宽度>` | 图片最大宽度 px，默认 `677`（公众号正文列宽）；`full` 表示与文字同宽 |
| `--force` | 覆盖已存在的同名文件 |
| `--print` | 只输出内容到标准输出，不写文件 |
| `--json` | 结构化结果：`title / account / author / published / state / path / characters` |

- 多个链接可以一次传入，共用一个输出目录。
- 用户没说存哪里 → 用 `-o ~/Documents/公众号文章`，并在回复里告知路径。
- 默认不覆盖同名文件；用户明确要求覆盖时加 `--force`。
...
- `WECHAT_CHALLENGE`：微信要求验证。让用户在浏览器打开链接完成验证后重试，不要尝试绕过。
- `CONTENT_MISSING`：页面没返回正文（链接失效或纯 JS 渲染页面）。如实告知，不要编造内容。
- `EXISTS`：文件已存在。确认后再决定是否用 `--force`。
- `state: partial`：付费文章，只有试读部分，回复时说明。
